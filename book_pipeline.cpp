...
private:
    ...
    const string dir;                         // directory with images
...
private:
    void findImagesInDiretcory(const string& dir) {
        ...
        utils::fs::glob(dir, "*.JPG", imagesFilenames);
        utils::fs::glob(dir, "*.jpeg", imagesFilenames);
        utils::fs::glob(dir, "*.JPEG", imagesFilenames);
...
//            for (size_t j = i + 1; j < imagesFilenames.size(); ++j)
                ...
                if (saveDebugVisualizations) {
                    ...
                    for (size_t i = 0; i< showList.size(); i++) {
                        drawMatches(images[imgi], keypoints[imgi],
                                    images[imgj], keypoints[imgj], showList[i].second, out, CV_RGB(255,0,0));
    ...
    void buildTracks() {
        ...
        // Remove vertices without any connection
    ...
    bool reconstructFromTracks() {
        ...
        if (Rs.size() != imagesFilenames.size()) {
            CV_LOG_INFO(&TAG, "Unable to reconstruct all camera views (" + to_string(imagesFilenames.size()) + ")");
            return false;
        }

        if (tracks[0].cols != points3d.size()) {
            CV_LOG_INFO(&TAG, "Unable to reconstruct all tracks (" + to_string(tracks[0].cols) + ")");
        }
    ...
    void visualize3D() {
        ...
        if (saveDebugVisualizations) {
            ...
            for (int j = 0; j < imagesFilenames.size()-1; j++) {
